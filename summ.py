...
def extract_text(file):
    ...
    if file.name.endswith(".csv"):  # Handle CSV files
        df = pd.read_csv(file)
        text_data = df.to_string()
    elif file.name.endswith(".pdf"):  # Handle PDF files
        reader = PyPDF2.PdfReader(file)
        for page in reader.pages:
            text_data += page.extract_text() + " "
    else:  # Handle raw text files
...
def summarize_text(text_data, length_ratio):
    """Summarizes text using a transformer-based model with user-defined length."""
...
def analyze_sentiment(text_data):
    """Performs sentiment analysis on the entire text using a transformer-based model."""
...
def named_entity_recognition(text_data):
    ...
    for entity in entities:
        entity_name = entity['word']  # Merged subwords are already handled
